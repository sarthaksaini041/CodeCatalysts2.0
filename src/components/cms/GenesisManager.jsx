        ...
        <div className="space-y-12">
            <div className="bg-white/[0.02] border border-white/5 rounded-[32px] p-8">
                ...
                <div className="flex gap-4 items-end">
                    ...
                    <button onClick={handleSaveTitle} disabled={isSaving} className="px-6 py-3 bg-primary text-black font-black text-[10px] uppercase tracking-widest rounded-xl hover:scale-105 transition-all text-white font-bold">SAVE_TITLE</button>
                </div>
            </div>

            <div className="space-y-6">
                <div className="flex justify-between items-center px-2">
                    <h3 className="text-xs font-black uppercase tracking-widest text-white/40">STORY_CHRONICLES</h3>
                ...
                <div className="grid grid-cols-1 gap-4">
                    {items.map((item, index) => (
                        <div key={item.id} className="group flex items-center gap-6 bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 rounded-2xl p-4 transition-all">
                            <div className="w-24 h-16 rounded-lg bg-white/5 overflow-hidden flex-shrink-0 border border-white/5">
                                {item.image_url ? (
                            ...
                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => moveItem(index, -1)} className="p-2 hover:text-primary transition-colors text-white/20"><ArrowUp size={16} /></button>
                                <button onClick={() => moveItem(index, 1)} className="p-2 hover:text-primary transition-colors text-white/20"><ArrowDown size={16} /></button>
                                <button onClick={() => setEditingItem(item)} className="p-2 hover:text-primary transition-colors text-white/20"><Edit3 size={16} /></button>
                                <button onClick={() => handleDeleteItem(item.id)} className="p-2 hover:text-red-500 transition-colors text-white/20"><Trash2 size={16} /></button>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {editingItem && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-[#050505]/80 backdrop-blur-md">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#0a0a0a] border border-white/10 rounded-[32px] w-full max-w-2xl p-8 shadow-2xl maxHeight-[90vh] overflow-y-auto">
                            <form onSubmit={handleSaveItem} className="p-2 space-y-6 text-white">
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="text-sm font-black uppercase tracking-widest text-primary">EDIT_STORY_POINT</h3>
                                    <button type="button" onClick={() => setEditingItem(null)} className="text-white/40 hover:text-white"><X size={20} /></button>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div className="space-y-4">
                                        <div className="space-y-2">
                                            ...
                                            <textarea name="description" defaultValue={editingItem.description} required rows={5} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary transition-all text-white" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-4 pt-4">
                                    ...
                                    <button type="submit" disabled={isSaving} className="flex-1 py-4 bg-primary text-black rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-primary/20 transition-all font-bold">SAVE_CHANGES</button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
